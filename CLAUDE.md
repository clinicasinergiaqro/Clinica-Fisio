...
- **Fase 1 (VALIDADA y a TODA la clínica — 2026-09-18, PR #348):** en cada guardado de activo, tras el POST OK al Sheet, `saveDB` llama `_mirrorHistoriaActivoFS(p, opts.camposHist)` que reusa `_persistirHistoricoFirestore(p, camposHist, /*silencioso*/true)` escribiendo a `.doc(p.id)`. Aditivo/fire-and-forget; falla → `_encolarHistDoc` (COLA_SYNC_HIST_DOC). Se cubrió el gap de `motivoHC`. NO toca la lectura.
