...
- **Fase 3 (EN CURSO, por sub-pasos — decisión de Carlos: prep segura ya, flip después):**
  ...
  - **3b (SIGUIENTE):** leer lista de activos de Firestore con Sheet de RESPALDO automático (fallback → nunca pantalla vacía), detrás de flag, probado SOLO en equipo de Carlos. Necesita: ampliar `pacienteClinicoUpdateOK` para `terapeutaSeguimiento` (y quizá `date`) o proyectarlos de otra fuente; extender proyección del índice; respetar ambos tombstones; conservar sentinel null.
